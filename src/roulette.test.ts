...
function createSettings8(
  ...
  options: Partial<Pick<AdvancedModeSettings8, 'noJobDuplicates' | 'maintainStandardComposition' | 'maintainAllianceComposition'>> = {}
  ...
  return {
    ...
    maintainAllianceComposition: options.maintainAllianceComposition ?? false,
  };
}
...
function countBaseRoles(members: { role: string }[]) {
  const count = {
    tank: 0,
    healer: 0,
    dps: 0,
  ...
  for (const member of members) {
    const role = member.role as keyof typeof count;
    if (role in count) count[role]++;
  }
  return count;
}
...
describe('アライアンス構成維持', () => {
  it('8人の汎用枠を1T2H5DPSに解決する', () => {
    vi.spyOn(Math, 'random').mockReturnValue(0);
    const settings = createSettings8(
      ['free', 'free', 'free', 'free', 'free', 'free', 'free', 'free'],
      {
        maintainStandardComposition: false,
        maintainAllianceComposition: true,
      }
    );
    ...
    expect(countBaseRoles(result.members)).toEqual({
      tank: 1,
      healer: 2,
      dps: 5,
    });
