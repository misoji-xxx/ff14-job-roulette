...
export const ALLIANCE_COMPOSITION_8: PartyComposition = {
  tanks: 1,
  healers: 2,
  dps: 5,
};
...
export function canMaintainAllianceComposition(settings: AdvancedModeSettings): boolean {
  if (settings.partySize !== 8) {
    return false;
  }

  const fixed = countFixedRoles(settings);
  return (
    fixed.tank <= ALLIANCE_COMPOSITION_8.tanks &&
    fixed.healer <= ALLIANCE_COMPOSITION_8.healers &&
    fixed.dps <= ALLIANCE_COMPOSITION_8.dps
  );
}
...
function resolveAllSlotRoles(
  ...
  return resolve8PlayerSlots(settings, resolvedRoles, maintainStandard, maintainAlliance);
}
...
function resolve8PlayerSlots(
  ...
  if (!maintainStandard) {
    if (maintainAlliance) {
      return resolve8PlayerAllianceSlots(settings, resolvedRoles, {
        genericHealerIndices,
        genericDpsIndices,
        freeIndices,
      });
    }
...
function resolve8PlayerAllianceSlots(
  settings: AdvancedModeSettings,
  resolvedRoles: (ResolvedRole | null)[],
  indices: {
    genericHealerIndices: number[];
    genericDpsIndices: number[];
    freeIndices: number[];
  }
): ResolvedRole[] {
  const healerSubRoles: ResolvedRole[] = ['pureHealer', 'barrierHealer'];
  ...
  const neededBaseRoles: Role[] = [];
  for (let i = 0; i < ALLIANCE_COMPOSITION_8.tanks - fixed.tank; i++) neededBaseRoles.push('tank');
  for (let i = 0; i < ALLIANCE_COMPOSITION_8.healers - fixed.healer; i++) neededBaseRoles.push('healer');
  for (let i = 0; i < ALLIANCE_COMPOSITION_8.dps - fixed.dps; i++) neededBaseRoles.push('dps');

  for (const idx of indices.genericHealerIndices) {
    resolvedRoles[idx] = getRandomItem(healerSubRoles);
  }

  for (const idx of indices.genericDpsIndices) {
    resolvedRoles[idx] = getRandomItem(dpsSubRoles);
  }

  const shuffledNeeded = shuffle(neededBaseRoles);
  let neededIdx = 0;

  for (const idx of indices.freeIndices) {
    const baseRole = shuffledNeeded[neededIdx++] ?? getRandomItem(['tank', 'healer', 'dps'] as Role[]);
    if (baseRole === 'tank') {
      resolvedRoles[idx] = 'tank';
    } else if (baseRole === 'healer') {
      resolvedRoles[idx] = getRandomItem(healerSubRoles);
    } else {
      resolvedRoles[idx] = getRandomItem(dpsSubRoles);
    }
...
export function runAdvancedRoulette(settings: AdvancedModeSettings): AdvancedRouletteResult {
  const effectiveMaintainAlliance =
    settings.partySize === 8 &&
    settings.maintainAllianceComposition &&
    canMaintainAllianceComposition(settings);
  const effectiveMaintainStandard =
    !effectiveMaintainAlliance &&
    settings.maintainStandardComposition &&
    canMaintainStandardComposition(settings);

  const resolvedRoles = resolveAllSlotRoles(
    settings,
    effectiveMaintainStandard,
    effectiveMaintainAlliance
  );
